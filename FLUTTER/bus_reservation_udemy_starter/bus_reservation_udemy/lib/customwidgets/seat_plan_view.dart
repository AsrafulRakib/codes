                            ...
                            onSelect: (value) {
                              if(value){
                                if(selectedSeats.length<4){
                                  selectedSeats.add(seatArrangement[i][j]);
                                  if(selectedSeats.length==4){
                                    showMsg(context, 'Maximum limit reached!');
                                  }
                                }else{

                                }
                              }else{
                                selectedSeats.remove(seatArrangement[i][j]);
                              }
...
class _SeatState extends State<Seat> {
  ...
  Widget build(BuildContext context) {
    return InkWell(
      onTap: widget.isBooked || widget.seats.length>4
          ? null : () {
              setState(() {
                selected = !selected;
